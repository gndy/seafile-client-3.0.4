...
void CloudView::setupHeader()
{
    ...
    mMinimizeBtn->setIcon(awesome->icon(icon_minus, QColor("#fff")));
    connect(mMinimizeBtn, SIGNAL(clicked()), this, SLOT(onMinimizeBtnClicked()));

    mCloseBtn->setText("");
    mCloseBtn->setToolTip(tr("Close"));
    mCloseBtn->setIcon(awesome->icon(icon_remove, QColor("#fff")));
...
void CloudView::updateAccountMenu()
{
    ...
    // Add rest items
    add_account_action_ = new QAction(tr("Login"), this);
    ...
    if (hasAccount()) {
        delete_account_action_ = new QAction(tr("Log Out"), this);
...
void CloudView::createToolBar()
{
    ...
    tool_bar_->setStyleSheet("QWidget{background-image:url(:/images/toolbar-bg.png);background-repeat: repeat-x;}");
