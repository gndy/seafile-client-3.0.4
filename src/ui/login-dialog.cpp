...
const QString kDefaultServerAddr1 = "http://www.dnayun.cn";
...
LoginDialog::LoginDialog(QWidget *parent) : QDialog(parent)
{
    ...
    //mServerAddr->addItem(kDefaultServerAddr2);wenlei
    mServerAddr->setEditText("http://www.dnayun.cn");//wenlei+
    mServerAddr->clearEditText();
    mServerAddr->setAutoCompletion(false);

    // hidden elements
    serverUrl->setVisible(false);
    mServerAddr->setVisible(false);
...
{
    QString serverAddr = kDefaultServerAddr1;
    QString protocol;
    QUrl url;
